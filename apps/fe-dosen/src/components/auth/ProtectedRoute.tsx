...
/** Map of role -> correct portal URL */
...
/**
 * Protected Route wrapper for fe-dosen
 * Redirects to login if not authenticated
 * Checks for 'dosen' role by default
 *
...
export function ProtectedRoute({ 
  ...
  if (requiredRoles && requiredRoles.length > 0) {
    ...
    if (!hasRequiredRole) {
      // Find the correct portal for the user's role
      const correctPortal = roles
        .map(r => ROLE_PORTAL_MAP[r])
        .find(url => url && url !== window.location.origin)?.replace(/^https?:\/\/[^/]+/, '') || null;
      ...
      return (
        <div className="min-h-screen flex items-center justify-center bg-gray-50">
          <div className="text-center bg-white p-8 rounded-xl shadow-lg border border-gray-100 max-w-md w-full">
            ...
            <p className="text-gray-700 mb-4">Anda tidak memiliki izin untuk mengakses Portal Dosen.</p>
