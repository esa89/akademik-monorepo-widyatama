...
export function ProtectedRoute({ children }: ProtectedRouteProps) {
  ...
  const requiredRoles: UserRole[] = ['jurusan', 'kaprodi'];
  if (!hasAnyRole(requiredRoles)) {
    const correctPortal = roles.map((r) => ROLE_PORTAL_MAP[r]).find((url) => url && url !== window.location.origin) || null;
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center bg-white p-8 rounded-xl shadow-lg border border-gray-100 max-w-md w-full">
          ...
          <p className="text-gray-700 mb-4">Anda tidak memiliki izin untuk mengakses Portal Jurusan.</p>
          <p className="text-sm text-gray-500 mb-6 bg-gray-50 p-3 rounded-lg border">
            ...
            <br />Role yang diperlukan: <span className="font-semibold text-gray-700">{requiredRoles.join(', ')}</span>
          </p>
          <div className="space-y-3">
            {correctPortal && (
              <a href={correctPortal} className="flex items-center justify-center gap-2 w-full px-4 py-2.5 bg-primary text-white rounded-lg hover:bg-primary/90 transition-colors font-medium">
                Buka Portal Anda <ArrowRight className="w-4 h-4" />
              </a>
            )}
            <button onClick={() => { localStorage.clear(); logout().catch(() => {}); window.location.href = '/login'; }}
